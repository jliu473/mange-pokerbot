...
class Player(Bot):
    ...
    def handle_round_over(self, game_state, terminal_state, active):
        ...
        if self.round_num <= 3.5 * self.round_test:
            self.winnings[(self.round_num-1) // self.round_test] += my_delta

            if self.round_num == self.round_test: # switch to tight at round 100
                self.raise_threshold = 0.8
                self.call_threshold = 0.7

            if self.round_num == 2 * self.round_test: # switch to loose at round 200
                self.raise_threshold = 0.6
                self.call_threshold = 0.5

            if self.round_num == 3 * self.round_test: # choose the best at round 300
                print(self.winnings)
                if self.winnings[0] == max(self.winnings):
                    self.raise_threshold = 0.7
                    self.call_threshold = 0.6
                elif self.winnings[1] == max(self.winnings):
                    self.raise_threshold = 0.8
                    self.call_threshold = 0.7
                else:
                    self.raise_threshold = 0.6
            ...
            if self.round_num == 3.5 * self.round_test: # test intimidation
                print(self.winnings)
                if 2 * self.winnings[3] < max(self.winnings[:3]):
                    self.intimidation = 0.3
