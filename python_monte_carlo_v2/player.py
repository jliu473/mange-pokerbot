...
class Player(Bot):
    ...
    def handle_round_over(self, game_state, terminal_state, active):
        ...
        self.opp_aggression += self.total_opp_bet / max(self.num_opp_bet, 1) - self.total_my_bet / max(self.num_my_bet, 1)
        print(self.opp_aggression)
    ...
    def get_action(self, game_state, round_state, active):
        ...
        strength = 0
        if self.round_num > 50 and self.opp_aggression > 0:
            strength += -self.opp_aggression / (self.round_num * 10)
        if won_auction == None:
        ...
        if continue_cost > 0:
            self.total_opp_bet += continue_cost
            self.num_opp_bet += 1
        ...
        if my_action == RaiseAction(raise_ammt):
            print("happens")
            self.total_my_bet += raise_ammt
            self.num_my_bet += 1
