...
class Player(Bot):
    ...
    def handle_round_over(self, game_state, terminal_state, active):
        ...
        if self.round_num <= 300:
            self.winnings[(self.round_num-1) // 100] += my_delta

            if self.round_num == 100: # switch to tight at round 100
                self.raise_threshold = 0.8
                self.call_threshold = 0.7

            if self.round_num == 200: # switch to loose at round 200
                self.raise_threshold = 0.6
                self.call_threshold = 0.5

            if self.round_num == 300: # choose the best at round 300
                print(self.winnings)
